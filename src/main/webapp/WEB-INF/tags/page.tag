	...
	<div class="container">
		<div class="header">
			<ul class="nav nav-pills pull-right">
				...
				<li <c:if test="${fn:endsWith(uri,'/')}"> class="active"</c:if>>
				...
				<c:choose>
					<c:when test="${!empty account}">
						<li <c:if test="${fn:endsWith(uri,'dashboard')}"> class="active"</c:if>>
							<a href="${pageContext.request.contextPath}/dashboard">Dashboard</a>
						</li>
						<li><a href="${pageContext.request.contextPath}/logout">Logout</a></li>
					</c:when>
					<c:otherwise>
						<li	<c:if test="${fn:endsWith(uri,'login')}"> class="active"</c:if>>
